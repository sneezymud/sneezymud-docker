...
export const account = mysqlTable("account", {
  account_id: bigint({ mode: "number", unsigned: true }).primaryKey(),
  name: varchar({ length: 80 }).notNull(),
  passwd: varchar({ length: 255 }).notNull(),
});

export const player = mysqlTable("player", {
  account_id: bigint({ mode: "number", unsigned: true }),
...
export const wizdata = mysqlTable("wizdata", {
  ...
  blockbend: int(),
  blockbstart: int(),
  player_id: bigint({ mode: "number", unsigned: true }).notNull().primaryKey(),
});

export const wizpower = mysqlTable(
  "wizpower",
  {
    player_id: bigint({ mode: "number", unsigned: true }).notNull(),
    wizpower: int().notNull(),
  },
  (table) => [primaryKey({ columns: [table.player_id, table.wizpower] })],
);
...
export const zone = mysqlTable("zone", {
  ...
  top: int(),
  util_flag: int(),
  zone_enabled: int().notNull(),
  zone_name: varchar({ length: 255 }).notNull(),
