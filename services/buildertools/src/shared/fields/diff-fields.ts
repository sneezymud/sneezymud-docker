...
function arrayItemFormatter(value: unknown): string {
  ...
  if (typeof value === "object") {
    ...
    if ("direction" in obj && "destination" in obj) {
      return `dir ${stringifyUnknown(obj["direction"])} -> room ${stringifyUnknown(obj["destination"])}`;
    }
    // Room/obj extras: show keyword
    if ("name" in obj && "description" in obj && !("keyword" in obj)) {
      return `"${stringifyUnknown(obj["name"])}"`;
    }
    // Mob extras: show keyword
    if ("keyword" in obj) {
      return stringifyUnknown(obj["keyword"]);
    }
    // Obj affects: show type and mod
    if ("type" in obj && "mod1" in obj) {
      return `type ${stringifyUnknown(obj["type"])}: ${stringifyUnknown(obj["mod1"])}/${stringifyUnknown(obj["mod2"] ?? 0)}`;
    }
    // Mob immunities: show type and amount
    if ("type" in obj && "amt" in obj) {
      return `type ${stringifyUnknown(obj["type"])}: ${stringifyUnknown(obj["amt"])}%`;
    }
    return JSON.stringify(value);
  }
