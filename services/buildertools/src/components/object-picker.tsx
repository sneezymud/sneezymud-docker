...
export function ObjectPicker({ id, onChange, value }: ObjectPickerProps) {
  const autoId = useId();
  ...
  const { data: results } = useQuery({
    ...
    queryFn: () =>
      apiFetch(
        `/api/objects/search?q=${encodeURIComponent(search)}`,
        objectSearchSchema,
      ),
    queryKey: objectKeys.search(search),
    staleTime: 30_000,
  });

  const items = results ?? [];

  const selectObject = (vnum: number) => {
    onChange(vnum);
    setOpen(false);
  };
  ...
  return (
    ...
    >
      ...
      <Popover.Portal>
        <Popover.Content
          align="start"
          className="z-20 w-72 rounded border border-zinc-700 bg-zinc-800 p-2 shadow-lg"
          onOpenAutoFocus={(e) => {
            e.preventDefault();
